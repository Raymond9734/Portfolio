  ...
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      ...
      >
        ...
        <div className='mt-5'>
          <h3 className='font-bold text-[24px]' style={{ color: styles.whiteColor }}>{name}</h3>
          <p className='mt-2 text-[14px]' style={{ color: styles.hoverDarkWhiteTxtColor }}>{description}</p>
        </div>
        <div className='mt-4 flex flex-wrap gap-2'>
          {tags.map((tag) => (
            <p key={tag.name} className={`text-[14px]`} style={{ color: generateRandomColor() }}>
              #{tag.name}
            </p>
          ))}
        </div>
